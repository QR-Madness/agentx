...
- **`POST /voice-command`** — body `{conversation_id, transcript, agent_name?, artifacts?}`. **Voice-mode intent routing:** the ambassador interprets a spoken command and returns `{action: "answer"|"relay"|"tool", text, qa_id?, tool?}` — a question it answers (spoken; persisted to the `qa:` sidecar so the Text tab shows it), an instruction it drafts as a first-person **relay** the user reviews and sends into the conversation as a real user turn (never auto-sent), or a conversation-management command (rename/archive/delete) it files as a **`tool` proposal** (`tool: {proposal_id, action, conversation_id, title?}`) the user confirms on screen — nothing executes here; the client applies a confirmed proposal via `PATCH /api/memory/conversations/{id}/meta` (or the conversation DELETE). Never fails the call (degrades to a spoken notice; an unknown tool or invalid args degrade to an `answer`). `text` for a `relay` is shaped via the draft persona; the JSON is forward-compatible with a `target` for future cross-agent delegation.
