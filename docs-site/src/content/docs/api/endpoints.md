...
Same request body as `/agent/chat`. Optional extras: `workflow_id` (run an Agent Alloy workflow — a "Team" in the UI), `target_agent_id` (route this turn to a specific agent by its Docker-style `agent_id`), `disable_delegation` (Solo mode — suppress ad-hoc delegation for this turn: no `delegate_to` tool, no roster prompt; ignored when `workflow_id` is set), `research_mode` (Research Mode — elevated per-turn search budget + a rigorous, evidence-grounded, self-reviewing research prompt that lands a durable cited report in the attached Project; forces the flat single-agent path; gated server-side by `research.enabled`), `thinking_pattern` (Thinking Patterns — per-turn override: `native` | `cot` | `step_back` | `reflection` | `deep_reflection` | `self_consistency`, legacy `tot`/`react` degrade honestly; unset ⇒ profile/auto chain; gated by `reasoning.chat_patterns_enabled`), `workspace_id` (attach a document workspace), `images` (**vision input** — an array of `{workspace_id, doc_id, media_type}` refs to images already uploaded via `POST /agent/chat/images`; a vision-capable model receives them as image blocks, a non-vision model gets the text only with a `status` notice), and `audio` (**audio input** — the same ref shape plus an optional cached `transcript`; capability-driven dual path: an audio-capable model hears natively-accepted containers ≤10MB as `input_audio` blocks, anything else is transcribed server-side via the shared Ambassador STT seam and inlined as a labeled transcript with `status: transcribing`/`audio_fallback` notices — the transcript persists onto the turn's metadata and is honored on resend, so a regenerate never re-transcribes). A media-only turn may send an empty `message`. Routing priority: `workflow_id` > `target_agent_id` > `agent_profile_id` > default; an unknown `target_agent_id` yields an `error` event. Returns Server-Sent Events (SSE).
...
| `done` | `{"task_id": "...", "thinking": "...", "has_thinking": bool, "total_time_ms": float, "session_id": "...", "context_window": int, "context_used": int, "context_summarized": bool, "context_dropped_turns": int, "thinking_pattern": "cot"|null, ...}` | Generation complete. `thinking_pattern` = the Thinking Pattern that shaped this turn (null = plain). `context_summarized` = a rolling summary covers older turns; `context_dropped_turns` = verbatim turns the ledger dropped this turn (0 when everything fit) — both feed the client's context chip |
...
The **Ambassador** (Phase 16.6/16.7, v3) is a dedicated agent that runs *parallel* to your conversations — a conversational operator with its **own persistent thread** (an "Inquiry"), never a reaction to your last turn. It observes the conversation world through a curated **tool belt that never executes a write**: reads (summarize/read/survey conversations, the structured conversation state, what a conversation *produced* — its exhibits and cited sources, full-text search across all conversations, live runs + queued jobs, the usage ledger, long-term memory recall, the agent roster) auto-run, while conversation-management writes (rename/archive/delete a conversation) **and task dispatches** (`dispatch_task` — hand a self-contained task to a worker agent) only **file a proposal** you confirm on screen — the write executes solely via the user-confirmed meta/dispatch endpoints, a confirmed dispatch landing as *your* user turn. Its answers are **grounded in facts**: aide digests preserve load-bearing specifics verbatim (a `Facts:` tail that survives clipping), it drills back into the source conversation when a summary can't answer a specific, it distinguishes what it read directly from what a digest reports — and a reasoning model's `<think>` blocks are stripped before they can reach the thread, the replay, or the voice. It briefs you on demand, relays messages into conversations as *your* turns, and dispatches tasks to workers — all **without polluting** any conversation (it writes only to its Redis sidecar, never `conversation_logs` or the rolling summary). It is any agent profile with an `ambassador` section; the global default is `config.ambassador.profile_id` (falls back to the default agent profile). It mounts both against a conversation and as the standalone **Command Deck** (a conversation-less per-user thread for cross-conversation questions).
...
- **`POST /dispatch`** — body `{agent_id, text, conversation_id?}`. The ambassador **write-side** (hand a task to a worker): runs the chosen worker (resolved by `agent_id`, agents only) **headless** on `text` as a **user** turn (`enqueue_background_chat`). By default a **brand-new conversation** is minted; pass `conversation_id` to run the task inside an existing conversation instead (unknown id → `404`, the relay guard). Returns `{ok, conversation_id, job_id}` so the client opens + watches it — the client echoes the task into the tab **immediately** (a seeded user bubble; the worker's reply lands on reload). You authored the task — the ambassador never speaks into the transcript as itself (INV-2). Gated by `config.ambassador.dispatch.enabled` (default on; `422` when off); unknown/non-worker `agent_id` → `400`. Confirm-first, reached two ways: the composer's dispatch mode (you pick the worker + send) or confirming an ambassador-filed `dispatch_task` proposal.
- **`POST /relay`** — body `{conversation_id, text}`. Deliver a relayed message into **any** conversation as a real **user** turn — even one with no open client tab — by running its agent **headless** via the background-chat worker (warm-hydrates, appends the user turn + the agent's reply to the durable history). The person is the author; the ambassador never speaks into the transcript. The target agent is recovered from the conversation's most recent stamped turn (`latest_agent_name` → `get_profile_by_name`), else the default profile. Returns `{ok, job_id}`; unknown conversation → `404`. The client (`lib/ambassadorRelay.ts::planRelay`) prefers the live in-tab handler when the target is the active tab and uses this endpoint otherwise (command deck, off-tab focus).
- **`GET /threads`** / **`POST /threads`** — the **command deck's** standalone Inquiries. `GET` lists the user's threads newest-first (`{threads: [{thread_id, title, created_at?, updated_at?}], deck_thread_id}` — the home `deck:{user}` thread is always present and pinned first); `POST` mints + registers a new standalone Inquiry (`inq:{user}:{uuid}`) → `{thread_id}` (titled later via `PATCH /thread/{id}` or auto-titled from its first question). Chat-bound Inquiries are reached through their conversation id, not here. Backed by a per-user Redis registry (`amb_user:{user}:threads`, self-healing).
- **`POST /voice-command`** — body `{conversation_id, transcript, agent_name?, artifacts?}`. **Voice-mode intent routing:** the ambassador interprets a spoken command and returns `{action: "answer"|"relay"|"tool", text, qa_id?, tool?}` — a question it answers (spoken; persisted to the `qa:` sidecar so the Text tab shows it), an instruction it drafts as a first-person **relay** the user reviews and sends into the conversation as a real user turn (never auto-sent), or a conversation-management / dispatch command (rename/archive/delete, or "have the researcher dig into…") it files as a **`tool` proposal** (`tool: {proposal_id, action, conversation_id?, title?, agent_id?, agent_name?, task?}`, `action` now including `dispatch`) the user confirms on screen — nothing executes here; the client applies a confirmed proposal via `PATCH /api/memory/conversations/{id}/meta`, the conversation DELETE, or `POST /dispatch` for a dispatch. Never fails the call (degrades to a spoken notice; an unknown tool or invalid args degrade to an `answer`). `text` for a `relay` is shaped via the draft persona; the JSON is forward-compatible with a `target` for future cross-agent delegation.
...
- **`GET /stream?run_id=`** — tail a briefing **or Q&A** run's namespaced SSE: `ambassador_start`, `ambassador_chunk` (one per streamed delta), `ambassador_tool_call` (`{message_id, tool, args}`) / `ambassador_tool_result` (`{message_id, tool, ok}`) — the agentic answer core driving its tool belt; rendered as live tool chips and persisted on the thread entry so they survive a reload. A confirmed-write tool (rename/archive/delete a conversation, or a `dispatch_task`) additionally emits **`ambassador_tool_proposal`** (`{message_id, tool, proposal: {proposal_id, action, conversation_id?, title?, agent_id?, agent_name?, task?}}`) between call and result — the belt only *files* the proposal (the tool result tells the model it's awaiting confirmation); the client renders a confirm strip (the proposal is also stamped on the persisted chip so a replay rebuilds it) and executes a confirmed one via `PATCH /api/memory/conversations/{id}/meta`, the conversation DELETE, or `POST /dispatch` for a dispatch. Then `ambassador_done` (status `done` | `empty_provider` | `cancelled`), `ambassador_error`; `run_missing` if the buffer expired. Cancel via `POST /api/agent/chat/runs/{run_id}/cancel` (settles the sidecar to `cancelled`, preserving partial text). A missing/unreachable provider degrades gracefully to an `empty_provider` notice rather than failing.
...
POST   /api/workspaces/{workspace_id}/avatars/prune                 # delete `avatars/` files no agent profile references → { deleted[], count }
...
```
GET   /api/conversations                          # ?limit= (max 100) &offset= &channel= &include_archived=
...
PATCH /api/memory/conversations/{conversation_id}/meta   # {title?, archived?} — user-set overlay (rename/archive)
```

**Response (list):** conversations summarized from `conversation_logs`, overlaid with
`conversation_meta` (Alembic 0007): `title` is the user-set custom title when one exists,
else the derived first-user-message title; `archived` conversations are **excluded by
default** from the rows *and* the pagination `total` (`?include_archived=1` opts back in).
```json
{
  "conversations": [
    {
...
**Conversation meta (`PATCH …/meta`):** body `{title?, archived?}` (at least one; `title`
non-empty ≤ 200 chars, `archived` boolean). The durable overlay behind rename + archive —
and the **execution half of the Ambassador's confirmed-write proposals**: the belt's
`rename_conversation` / `archive_conversation` tools only *file* proposals, and the client
calls this endpoint after you confirm. Meta-only — transcript turns are never touched.
Returns the applied fields; a failed persist is an honest `503` (never a phantom success).
...
| `/api/prompts/title` | POST | Generate a concise conversation title (state + first/last message) via the LLM |
...
Deletes a single conversation and its turns from all databases — plus its
`conversation_meta` row and its sidecar families (structured state, rolling summary,
and the ambassador's Inquiry thread; each best-effort — the response's
`deleted.sidecars` lists what cleared). Nothing strands.
