/**
 * PromptLibraryBrowser — Browse, search, select, and edit prompt templates.
 *
 * Shared surface rendered by both PromptLibraryModal (in-chat / profile-editor
 * quick access, with insert/select actions) and the unified-settings
 * PromptTemplatesSection (pure management surface — no insert action).
 *
...
import './PromptLibraryBrowser.css';

// ─── Types ────────────────────────────────────────────────────────────────────

export interface PromptLibraryBrowserProps {
  /** Close the hosting surface (modal/panel). Omit for embedded surfaces (settings section). */
  onClose?: () => void;
...
export function PromptLibraryBrowser({
  ...
  // Embedded in the settings section, SectionHeader already carries the title.
  const showTitle = variant !== 'section';
