...
describe('connectionString', () => {
  ...
  it('parses a code produced by the manager GUI encoder (cross-project parity)', () => {
    // Literal generated by manager/ui/src/lib/connect.ts — the deployment
    // manager's encode-only copy of this module. If this breaks, the wire
    // format changed in one place but not the other: fix BOTH and bump the
    // payload version together.
    const managerVector =
      'eyJ2IjoxLCJ1IjoiaHR0cHM6Ly9hZ2VudHguZXhhbXBsZS5jb20iLCJnIjoidG9rLWFiYzEyMyIsIm4iOiJmYW1pbHktY2x1c3RlciJ9';
    expect(parseConnectCode(managerVector)).toEqual({
      url: 'https://agentx.example.com',
      gatewayToken: 'tok-abc123',
      name: 'family-cluster',
    });
    expect(
      parseConnectFragment(`#connect=${managerVector}`)?.url,
    ).toBe('https://agentx.example.com');
  });
