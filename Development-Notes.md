...
**Multi-modal content protocol (v0.21.237).** One coherent slice generalizing the image plumbing above into
a versatile payload seam + full audio in/out (video render-only). **Content blocks
(`agentx_ai/content_blocks.py`):** a small pydantic union deliberately mirroring the **MCP ContentBlock**
vocabulary (which Zed's Agent Client Protocol reuses verbatim — there is no separate "Agent Content
Protocol"): `text`/`image`/`audio`/`resource_link`/`resource`, base64 `data` + `mimeType` (aliased),
`block_from_mcp`/`block_from_wire`/`.to_wire()`. It's a *seam*, not a refactor — existing paths keep their
shapes; MCP passthrough and provider media capture speak blocks, and a future ACP/A2A bridge is a
serializer, not a redesign. **MCP media passthrough (`mcp/media_passthrough.py`):** the tool executor now
converts `AudioContent` + `ResourceLink` (previously `"unknown"`-stringified), and `core._execute_tool_calls`
flattens via `flatten_result_content` — text-only results stay byte-identical to the legacy `.text`, while
image/audio blocks are **stored as workspace media** (attached ws else Home, `mcp/{server}-{tool}-…`; caps
≤3 blobs/result, ≤10MB each — untrusted input can't spam the disk) and the model-facing string becomes
`{"text", "stored_media":[served-blob refs], "resource_links"}` (b64 never burns context; the model can
`view_image` a stored image). **Provider media capture:** `StreamChunk.media`/`CompletionResult.media`
(content-block wire dicts) + `base.extract_media_blocks` — OpenRouter populates them from `message.images[]`
/ `delta` extension fields and `message.audio.data`, replacing the "completion carried an image the text
loop ignores" hole. The tool loop consumes `chunk.media` via `_emit_model_media` (store → `exhibit` SSE →
synthetic `present_exhibit` persistence turn), so an image/audio-output model answering *as* the agent —
directly or as a delegated specialist (the delegation path runs the same loop and forwards `exhibit` events
top-level) — renders without the image-only direct-mode detour (which remains for image-ONLY models).
**Audio input (capability-driven dual path):** `MediaRef` generalizes `ImageRef` (same fields + optional
cached `transcript`; `ImageRef = MediaRef` alias keeps every persisted turn parseable), `Message.audio`,
chat body `audio[]` (upload via the widened `/agent/chat/images` or its `/agent/chat/media` alias;
`kit/workspaces/service.py` splits `IMAGE_/AUDIO_/VIDEO_CONTENT_TYPES` with per-kind caps 25MB/100MB —
`MEDIA_CONTENT_TYPES` stays as the union). The native path (`_gate_audio_input`) requires model
`input_modalities` audio (`_model_accepts_audio`, catalog-warm mirror of the vision gate) AND an
OpenRouter-native container (`base.INPUT_AUDIO_FORMATS` — wav/mp3/ogg/m4a/aac/flac; **webm is not native**)
AND ≤10MB → `convert_messages_to_openai_format` emits `{type:"input_audio"}` blocks (Anthropic converter
deliberately skips audio). Everything else transcribes via the **Ambassador's STT seam**
(`get_ambassador().transcribe` — ONE model resolution with voice mode) into a labeled inline transcript,
with `status: transcribing`/`audio_fallback` notices; the transcript is **cached onto the ref → persisted
`metadata.audio[i].transcript`** so reload/regenerate never re-transcribes or re-bills. History strip mirror:
`_strip_message_audio`. Client: Relay-menu "Attach audio" + "Voice note" (records straight to **WAV** via
`lib/audioRecorder` — natively eligible; no MediaRecorder), `MessageAudio` chips (composer strip + user
bubble, transcript fold-out), pre-warning when the model can't hear (`input_modalities` via
`fetchModelsOnce`), settings under the renamed **Images & Audio** section (`audio.input_enabled`).
**Audio output:** `generate_speech` internal tool (`agent/audio_gen.py::generate_and_store_speech` —
Ambassador `synthesize(usage_source="chat_tts")` → `store_media` `generated/…mp3`) → **`audio` exhibit**;
`_emit_image_exhibit` generalized to `_media_exhibit_wires`/`_emit_media_exhibits` (generate_image /
generate_speech / MCP `stored_media`), and the same wires now persist as synthetic `present_exhibit` turns —
**fixing the pre-existing bug where a tool-generated image exhibit was lost on reload**. Gate:
`audio.speech_enabled`. **Exhibits:** new `AudioElement`/`VideoElement` (render-only; client `<video>`
degrades to a download link on codec failure)/`TextElement` (markdown through the chat `MessageContent`
pipeline — same sanitization) + **`grid` layout** (client 2-col, stacks on mobile; unknown layouts degrade
to stack) — all ADDITIVE, `schema_version` stays 1. **Hardening (retrofits image):** every media element
`url` must match the served-blob pattern (`_validate_served_blob_url`) — an agent-authored exhibit can never
...
- **Agent-profile editor** (`unified-profile-editor/`): hero identity header over a `ControlCard` grid; avatars in `common/AvatarPicker` (`lib/avatars.ts`) — an icon id **or** a generated image (the Generate tab is a **candidate deck**: subject optional w/ profile-seeded placeholder, empty subject → the template's synthetic-face branch, "Deal 4" sequential 2×2 grid, click-to-apply → `POST /api/agent/avatar/generate`, stored as `media:{ws}/{doc}` on `profile.avatar`; model = `images.avatar_model`); render via `common/AgentAvatar` (image avatars resolve to an authed object URL through `lib/avatarImage.ts`, icon otherwise). **NameDeck** (`NameDeck.tsx`): dice button on the hero name row → popover dealing 10 names from `GET /api/agent/names` (Random | Preferred pools, star/unstar via `/api/agent/names/preferred`; server store `agent/name_pools.py` → `data/name_pools.json`, deal-time exclusion of in-use names). **Chain strip** (`ChainStrip.tsx`, in the Team membership card): the agent's org placement as clickable avatar nodes (superior → agent → subordinates) derived via `lib/orgPlacement.ts` (client mirror of `alloy/org_chart.py` — manager-anchored `inOrg`, presentational superior/subordinate edges; vitest-pinned); node clicks hop the editor (`onSelectProfile` threaded from `UnifiedProfileEditor`). **ProfileNav v2**: org-grouped annotated roster (Manager / per-team lead-first / Org-free / Ambassador via `orgPlacement.groupRoster`; two-line rows w/ MGR/LEAD chips from workflow edges, model short-id, ≤2 tags; search over name/tags/model; "Manual order" header toggle keeps the dnd-kit flat list, persisted `agentx:profile-nav-mode`, reorder paused while filtering). Per-agent palette commands deliberately NOT added (useCommands' no-state-browser rule). Signature color from `lib/agentAccent.ts`. Primitives: `ui/SegmentedControl`, `ui/CopyChip`, `common/ControlCard`.
...
- **Chain of command (Agentic Organizations, Slices 1+2)**: profiles carry `org_level` (`executive` reserved / `manager` / `lead` / `agent`; **orthogonal to `kind`**) and workflows carry `manager_agent_id` (the owning manager; hard-validated resolvable/not-lead/not-member, tier mismatches warn-only). All derivation lives in **`alloy/org_chart.py`** (the single source AND the single test patch point): `chain_targets()` = adjacency only (`down_lead`/`down_member`/`up_lead`), `in_org()` is **manager-anchored** (a manager-less team's lead/members stay org-free — INV-10's back-compat linchpin). With `alloy.chain_of_command` (default ON): `list_adhoc_delegation_targets` returns relation-annotated chain rows for org participants (roster prompt swaps to `CHAIN_ROSTER_HEADER`, incl. the `[NEEDS ESCALATION]` mid-run convention; member ↑ lead escalation is just the ordinary ad-hoc tool with an `up_lead` enum), and `AlloyExecutor._validate_target` re-enforces adjacency + a categorical `delegation_path` loop guard (dual enforcement, INV-10). **Downward nesting**: a delegated specialist that leads a manager-owned team gets a members-scoped `delegate_to` + a **nested executor** (`_chain_nesting_for` — `base_depth`/`delegation_path`/`parent_delegation_id` as per-instance immutables since the tool loop omits those kwargs; blocking-only in v1) attached via `_active_alloy_executor`; its `delegation_*` events pass through the outer re-wrapper **unchanged** (client `buildDelegationTree` nests by `parent_delegation_id` — zero client work), nested completes own their exhibits (deduped from the outer set), and tool-loop metric capture keys on `tool_call_id` so nested completes can't clobber branch metrics. `org_level: manager` additionally applies the **report-only blocklist template** once on transition (`MANAGER_REPORT_ONLY_BLOCKED_TOOLS`, mirrored in `client/src/lib/managerTemplate.ts` — both pinned by tests) + a mandatory `manager_charter` prompt block (prio 92, `_append_team_blocks`). The default ambassador is delete-guarded (system-owned; INV-11).
- **Exhibits**: the internal `present_exhibit` tool emits a typed `exhibit` event (declarative Gallery→Exhibit→Element tree) instead of tool cards. Element types: `mermaid`, `choice` (clicking submits as the next user turn), `table` (sortable/scrollable/expand-to-modal), `citation` (active foldable sources vs passive links), `text` (markdown through the chat pipeline), and the media trio `image`/`audio`/`video` (served-blob `url` only — the pattern validator rejects external URLs; video is render-only). Layouts: `stack` (default) | `grid` (client 2-col, degrades to stack). A successful `web_search` auto-emits a passive `citation` exhibit (deduped by URL; id `exh_src_<tool_call_id>`; toggle `citations.auto_capture_web_search`); `generate_image`/`generate_speech` results and MCP media-passthrough `stored_media` auto-emit image/audio exhibits (`tool_loop._emit_media_exhibits`) **and persist as synthetic `present_exhibit` turns** (reload-safe — previously the tool-path image exhibit was lost on reload).
...
- **Turn-recovery guards** (v0.21.243, all in `streaming/tool_loop.py` unless noted; born of a live death-spiral: an OpenRouter route leaked tool-call NAMES into the content stream, the polluted transcript taught the model to "call" tools as plain text, and every retry deepened it): (1) **Tool-name echo strip** — a round that made structured tool calls has a leading called-name echo stripped from the accumulation (`_strip_tool_name_echo`; boundary rule: position 0, remainder empty or non-whitespace — prose *about* a tool survives); the client applies the same rule to the live buffer at its `tool_call` flush seam (`stripToolNameEcho` in `lib/messages.ts` — keep the two in sync). Live chunks already streamed are not rewritten; storage is truth and reload converges. (2) **Bare-tool-name recovery nudge** — a final reply that exactly equals an offered tool's name (or `name()`) is never a legitimate answer: the poison is dropped from the accumulation and a one-shot USER nudge (`chat.tool_name_recovery_nudge`) asks for a real call or a real answer. (3) **Informed final round** — the tools-withheld last round (`tool_round == max_tool_rounds`) now gets an explicit wrap-up USER message (`chat.tool_budget_wrapup`) + an `info {type: 'tool_budget_exhausted'}` event, instead of silently stripping tools and accepting mid-task narration as the "final" (the `for/else` forced-synthesis backstop is unchanged and still catches models that keep emitting tool calls). (4) **State-tool narration cap** — rounds whose ONLY calls are `update_conversation_state` count as narration-spin (`ToolLoopResult.state_solo_rounds`); past `STATE_TOOL_SOLO_ROUND_CAP` (3) further state calls short-circuit with an error TOOL result ("stop narrating; act, delegate, or finish") — every call id still gets its TOOL message (provider contract); mixed rounds and multi-slot single rounds never count. (5) **`NO_REASONING` utility knob** (`providers/base.py`) — `extra_body={"reasoning": {"enabled": false}}` on small-budget utility completions (title, thinking classifier, recall HyDE/self-query): a reasoning route burns a tiny `max_tokens` on hidden thinking and returns EMPTY content (live failure: the `gemini-flash-latest` alias drifted onto a thinker and auto-titling returned `""` at max_tokens=24). `/prompts/title` also gained headroom (256) + a deterministic `derive_title` fallback (never returns an empty title), and OpenRouter's non-stream `complete()` warns when content is empty but reasoning is present. (6) **Rate-limit wait-out** — a 429 on a round's model call that streamed NOTHING yet is retried through `RATE_LIMIT_WAIT_SCHEDULE` (15/30/60/120s; a bounded `Retry-After` header wins), with a visible status label per wait and 1s-step sleeps so Stop lands mid-wait (the SDK's own retries are silent and un-cancellable — observed as a 4-minute "hang"); background work orders keep running through the wait. (7) **Partial-turn salvage** — an unrecoverable round failure on a turn WITH substance (prior rounds/content/delegations/work orders) folds completed work-order reports, appends a bracketed "failed mid-turn — progress preserved" note, sets `finish_reason="error"`, and returns so the normal done/persistence path runs — a 429 mid-fan-out no longer destroys the specialists' finished work (a bare round-0 failure still raises into the views error event). The forced-synthesis pass has the same guard (falls into its existing empty fallback). (8) **Slow-start watchdog** (`_slow_start_pinger`) — while a round's first chunk hasn't arrived, a "Still waiting on the model (Ns)…" status pings at `SLOW_MODEL_FIRST_PING_SECONDS` then every `SLOW_MODEL_PING_INTERVAL_SECONDS`, covering the SDK-internal retry dead air. Guards: `StreamingToolLoopTest` (echo/nudge/wrap-up/cap/rate-limit/salvage/watchdog), `PromptsTitleTest`, `toolEcho.test.ts`.
