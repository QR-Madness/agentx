...
| `/api/agent/ambassador/stream` | GET | Tail a briefing/Q&A run (`?run_id=`): `ambassador_start`/`_chunk`/`_tool_call` (`{message_id, tool, args}`)/`_tool_proposal` (`{message_id, tool, proposal}` — a confirmed-write filing, executed only via the meta endpoints after user confirm)/`_tool_result` (`{message_id, tool, ok}`)/`_done`/`_error` SSE |
...
| `/api/agent/ambassador/dispatch` | POST | Hand a task to a chosen worker (`{agent_id, text}`): mints a **new** conversation, runs the worker headless on the task as its first user turn → `{ok, conversation_id, job_id}`; gated `ambassador.dispatch.enabled` (`422` off); non-worker `agent_id` → `400` |
