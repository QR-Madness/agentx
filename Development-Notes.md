...
The main streaming chat path. Runs **detached server-side** (survives client disconnect); first event `run_started` carries `run_id`. Events: `run_started, start, chunk, status, steer, tool_call, tool_result, exhibit, workspace_attached, done, error, close`.
