...
- [ ] Adopt the new primitives repo-wide (replace ad-hoc `.stat-badge`/`.active-badge`, `.card`, raw inputs, `.section-title` blocks) and migrate magic-number spacing to `--space-*`. **Started:** `memory-settings/` is the first adopter; raw `button-*` classes remain in ~18 other files. Natural next adopters of the `fields/` components: the unified-settings sections (`ProvidersSection`/`ModelsSection`/`PlannerSection`).
