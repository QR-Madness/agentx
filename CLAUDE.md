...
| `/api/agent/ambassador/thread/{thread_id}` | GET/PATCH | Replay one unified ambassador thread ("Inquiry") `{thread_id, title, entries}` (briefings + Q&A as one ordered list, persisted tool chips) / rename it (`{title}`). `thread_id` defaults to the conversation id |
