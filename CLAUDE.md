...
- Conversations: the chat page (`pages/AgentXPage.tsx`) is a **left `ConversationSidebar` rail + `ChatPanel`** (the old TopBar tab strip was removed). The rail is collapsible (persisted `agentx:conv-sidebar-collapsed`) and is presentation over `ConversationContext` (the tab/session/streaming model is unchanged — "open conversations" are the top rows). The list body + logic are shared via **`hooks/useConversationList.ts`** + **`components/chat/ConversationList.tsx`**, reused by the mobile **Conversations drawer** (`SURFACES.conversations`). Conversation-management tools (multi-select/bulk, pin, archive) plug into `ConversationList` rows next.
