...
- `content_blocks.py` — multi-modal payload vocabulary mirroring MCP/ACP ContentBlocks (text/image/audio/resource_link/resource, base64 `data`+`mimeType`); the seam shared by providers (`StreamChunk.media`), the MCP executor, and exhibits. Audio in/out rides it: `MediaRef` refs + capability-gated `input_audio` vs Ambassador-STT fallback; `agent/audio_gen.py` + `generate_speech` = the audio twin of `image_gen.py`/`generate_image`. ★
