...
Phases 1-14 and 17 (Server Management: auth, Docker production stack, multi-cluster, version matching) complete. Phase 15 (Plan Execution) ~80%. Phase 16 (Multi-Agent Conversations) ~40% — Agent Alloy v1 shipped (supervisor + specialist delegation); 16.1 per-turn attribution, 16.2 explicit agent routing, 16.3 per-agent tool isolation, and 16.4 ad-hoc agent-to-agent delegation (`delegate_to` generalized to non-workflow chats) shipped. Phase 18 (UX Improvements + Memory Tuning) in progress. Current version: 0.21.3 ("Mobile-Ready Alpha"). See `Todo.md` for detailed tracking.
