...
- **Agent-profile editor** (`unified-profile-editor/`) is a control-center: a hero identity header (inline name, `agent_id` `CopyChip`, kind/default `Badge`s, tags, description) over a `ControlCard` grid per tab. Avatars are consolidated into `common/AvatarPicker` (searchable, categorized modal over `lib/avatars.ts`'s `AVATAR_OPTIONS`/`AVATAR_CATEGORIES`; a disabled "Generate" seam for future AI icons). Each agent has a deterministic signature color from `lib/agentAccent.ts::agentAccent(agent_id)` (hero aura + badges). New shared primitives: `ui/SegmentedControl` (reasoning strategy, picker tabs), `ui/CopyChip`, `common/ControlCard`. All field logic + `useProfileEditorState` (autosave) unchanged — the redesign is presentation only.
- **Prompt Stack editor** (Settings → Intelligence → "System Prompt", `unified-settings/sections/SystemPromptSection.tsx` + `prompt-stack/{LayerCard,ComposedPreview,LayerDiffModal}`): a two-pane block composer over the `/api/prompts/layers` API — `@dnd-kit` drag-reorder, debounced-autosave inline edit, reset/diff (`diff` lib) for built-ins, live preview via `lib/promptStack.ts::composeStack` (client mirror of the backend join, unit-tested). Snippets from the **Prompt Library** (`template_manager`) insert as custom layers (`PromptLibraryModal` in `mode='insert'`), and the prompt **enhancer** (`/api/prompts/enhance`) rewrites a layer in place (with undo). The legacy `/prompts/global` editor never existed in the UI; this is its replacement
...
| `/api/agent/profiles`, `/api/agent/profiles/{id}` | GET/PATCH/DELETE | Agent profile CRUD; `.../set-default` (POST, agent); `.../set-default-ambassador` (POST, ambassador). Profiles carry `kind` (`agent`\|`ambassador`) |
