...
- `agent/ambassador.py` (+ `ambassador_storage.py`, `ambassador_tools.py`, `aide_swarm.py`, `conversation_meta.py`) — the **Ambassador**: parallel conversational operator with persistent "Inquiry" threads + the standalone **Command Deck**; a tool belt that **never executes a write** — reads auto-run (read/survey conversations, structured state, full-text search, live runs, usage, memory recall, roster; aide digests), conversation-meta writes (rename/archive/delete) are **proposal-only** (confirm strip → `PATCH /memory/conversations/{id}/meta`), and the write-side otherwise lands only as *your* user turns (relay/dispatch); sidecar-only, never pollutes the transcript. ★
