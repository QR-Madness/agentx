...
- `agent/ambassador.py` (+ `ambassador_storage.py`, `ambassador_tools.py`, `aide_swarm.py`) — the **Ambassador**: parallel conversational operator with persistent "Inquiry" threads + the standalone **Command Deck**; SELECT-only tool belt (read/survey conversations, roster; aide digests) whose write-side only lands as *your* user turns (relay/dispatch); sidecar-only, never pollutes the transcript. ★
