...
| `/api/agent/ambassador/brief-turn` | POST | **Ambassador** (16.6): start a parallel briefing of one turn (`{conversation_id, message_id, assistant_text, user_text?}`) → `{run_id}`. Detached + `indexed=False` (stays out of `/chat/runs`); writes only to the `ambassador:` sidecar (never the transcript). Cancel via `/chat/runs/{run_id}/cancel` |
| `/api/agent/ambassador/stream` | GET | Tail a briefing run (`?run_id=`): namespaced `ambassador_start`/`ambassador_chunk`/`ambassador_done`/`ambassador_error` SSE; `run_missing` on buffer expiry |
| `/api/agent/ambassador/{conversation_id}` | GET | Replay a conversation's persisted briefings from the sidecar (cold-open / reload / tab-switch) |
