...
| `/api/agent/chat/runs/{run_id}/steer` | POST | Live-steer a running turn (`{message, mode?}`): queues the message; the tool loop drains it at the next safe boundary (after a tool round, or instead of ending) and folds it in as a fresh user turn so the agent course-corrects without stopping. Owner-only; echoes a `steer` event onto the run bus so all clients show the steer bubble inline |
