...
- **Capability-aware advertisement**: the backends differ (Tavily = a research suite via the official `tavily-python` SDK: search/extract/map, plus crawl/research deferred; Brave = search-only via httpx). A pre-check (`resolve_active_search_backend` + `build_tool_schema`/`build_tool_description`) inventories the *active* backend at tool-listing time (`get_internal_tools`) and advertises only its real tools + knobs to the model. `web_extract`/`web_map` are advertised only when Tavily is active, but stay registered (executable) so a stale call self-guards. `SEARCH_CAPABILITIES` is the single source of truth.
- `web_search` (both backends) keeps results compact (no raw page content → stays under the oversize threshold + parseable for auto-capture); `web_extract`/`web_map` are Tavily-only and error cleanly without it. Config: `search.backend`, `search.{tavily,brave}_api_key`, `search.fallback_enabled`.
- A successful `web_search` auto-captures sources as a passive `citation` exhibit (see the `/api/agent/chat/stream` row); the shared `citation_exhibit_from_web_search` dedupe helper is the seed of the future conversation Bibliography.
