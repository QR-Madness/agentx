...
- **Settings that explain themselves** — **Recall** and **Conversation Context** say what each knob does and when to leave it alone; the same words appear in the new Settings Reference.
...
- **Agents read the web far more cheaply** — `web_extract` returns only the passages answering your query (3.8% of a real article), sub-questions batch, seen pages return as stubs. **Settings → Web Search** caps it: per-turn budgets in *calls and dollars*, plus preferred/blocked domains.
