...
- **Agents can run commands in a workspace (opt-in, sandboxed).** With `shell.enabled` on and the tool granted to an agent, the agent can run shell commands and read/write files against a **sandboxed working copy** of the attached workspace. Every command runs in a bubblewrap jail — **no network, no access to your secrets/`data/`, scrubbed env, time-limited** — so a prompt-injected agent can't exfiltrate or roam the host. Off by default; see the threat-model note in the docs.
