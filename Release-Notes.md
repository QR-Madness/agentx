...
- **Ambassador correctness.** Loading a conversation is pure display (no re-speaking/re-billing saved briefings); it names each conversation's own agent; the panel shows the ambassador's own name/avatar; voice shows the ambassador thinking, not your agent. **Voice relay now reaches the active conversation reliably and tells you where it landed** (no more silent "Send to agent" that went nowhere); read-only "summarize/explore" voice asks run automatically.
...
- **Model-capability awareness.** An agent on a model that can't use tools no longer 404s on its first turn — tools are simply not sent to it. The model picker now shows an **Image gen** badge (image *output*) alongside Vision (image *input*), so you can pick the right model for generation. Generated avatars render at full size now (they were collapsed to a dot on the desktop webview), and each agent reliably shows its *own* avatar in conversations — no more leaking across agents or falling back to the default star.
- **Direct mode for bare models.** New per-agent **Direct mode** (Settings → agent → Advanced) sends the model only your message — no system prompt, no memory, no tools. It's the right setting for a transform-only model (a fast classifier/rewriter) and is **auto-enabled for image-only models** (e.g. flux), which can't act on a full agent harness and otherwise just go quiet. The profile also now honors its own **memory toggle** in chat (previously only the request-level switch applied).
