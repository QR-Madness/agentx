...
- **The Ambassador is now a parallel operator.** A dedicated agent that runs *beside* your conversations (never in them). Its panel is an **Inquiry** you ask, brief, or talk to — with read-only tools to read across your conversations, **survey them all and summarize what your agents have been working on**, and know its **agent roster** — each agent's role, what its model can do (image/audio/vision/tools), and whether it's delegable (groundwork for multi-modal routing).
